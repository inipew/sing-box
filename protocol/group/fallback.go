...
func (s *Fallback) AttachConnection(closer io.Closer) func() {
	s.group.Touch()
	...
	for failedCount < len(candidates) {
		...
		case res := <-resultChan:
			if res.err != nil {
				...
				s.group.history.DeleteURLTestHistory(RealTag(res.outbound, network))
			} else {
				cancelAll()
				go func() {
					wg.Wait()
				...
				return s.group.interruptGroup.NewConn(res.conn, interrupt.IsExternalConnectionFromContext(ctx), interrupt.IsResourceDownloadFromContext(ctx)), nil
			}
		}
	}
