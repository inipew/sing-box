...
func (s *LoadBalance) Selected(network string) adapter.Outbound {
	return s.group.SelectTarget(context.Background(), network, M.Socksaddr{})
}
...
func (s *LoadBalance) DialContext(ctx context.Context, network string, destination M.Socksaddr) (net.Conn, error) {
	...
	if err == nil {
		...
		return s.group.interruptGroup.NewConn(wrappedConn, interrupt.IsExternalConnectionFromContext(ctx), interrupt.IsResourceDownloadFromContext(ctx)), nil
	}
	decrement()
...
func (s *LoadBalance) ListenPacket(ctx context.Context, destination M.Socksaddr) (net.PacketConn, error) {
	...
	if err == nil {
		...
		return s.group.interruptGroup.NewPacketConn(wrappedConn, interrupt.IsExternalConnectionFromContext(ctx), interrupt.IsResourceDownloadFromContext(ctx)), nil
	}
	decrement()
	s.logger.ErrorContext(ctx, err)
	s.group.history.DeleteURLTestHistory(outbound.Tag())
