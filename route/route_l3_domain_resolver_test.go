...
func TestPreMatchFlowResolvesSniffOverrideDestination(t *testing.T) {
	...
	metadata := adapter.InboundContext{
		Network:           N.NetworkUDP,
		Source:            M.ParseSocksaddr("192.0.2.1:1234"),
		OriginDestination: originalDestination,
		Destination:       M.ParseSocksaddr("example.com:443"),
		Domain:            "example.com",
		DestOverride:      true,
	}
...
func (g *testOutboundGroup) Type() string {
	return "selector"
}

func (g *testOutboundGroup) Tag() string {
	return "group"
}
...
func (g *testOutboundGroup) Selected(network string) adapter.Outbound {
	return g.selected
}
